...
@section('content')
    <form method="POST" action="{{ route('layout-settings.update') }}" class="space-y-8" enctype="multipart/form-data">
        @csrf
        @method('PATCH')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        @if (!empty($header['logo']))
                            ...
                            <img src="{{ Storage::url($header['logo']) }}" alt=""
                                class="mt-2 h-12 w-auto object-contain">
                        @endif
                        <x-input-error class="mt-2" :messages="$errors->get('header_logo_file')" />
                    </div>
                            ...
                            <div class="nav-link-item border rounded-md p-4 bg-gray-50">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <x-text-input name="header[nav_links][{{ $index }}][label]" type="text"
                                            class="mt-1 block w-full" :value="$link['label'] ?? ''" required />
                                    </div>
                                    <div>
                                        <x-input-label :value="__('URL')" />
                                        <x-text-input name="header[nav_links][{{ $index }}][url]" type="text"
                                            class="mt-1 block w-full" :value="$link['url'] ?? ''" required />
                                    </div>
                                </div>
                                    ...
                                    <div class="children-container space-y-2 ml-4">
                                        @foreach ($link['children'] ?? [] as $childIndex => $child)
                                            <div class="child-item grid grid-cols-2 gap-2">
                                                <x-text-input
                                                    name="header[nav_links][{{ $index }}][children][{{ $childIndex }}][label]"
                                                    type="text" class="block w-full" :value="$child['label'] ?? ''"
                                                    placeholder="Label" />
                                                <div class="flex gap-2">
                                                    <x-text-input
                                                        name="header[nav_links][{{ $index }}][children][{{ $childIndex }}][url]"
                                                        type="text" class="block w-full" :value="$child['url'] ?? ''"
                                                        placeholder="URL" />
                                                    <button type="button"
                                                        class="remove-child text-red-600 hover:text-red-800 px-2">&times;</button>
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
                                    <button type="button"
                                        class="add-child mt-2 text-sm text-indigo-600 hover:text-indigo-800">+ Add dropdown
                                        item</button>
                                </div>

                                <button type="button"
                                    class="remove-nav-link mt-3 text-sm text-red-600 hover:text-red-800">Remove
                                    link</button>
                            </div>
                        @endforeach
                    </div>
                    <button type="button" id="add-nav-link" class="mt-4 text-sm text-indigo-600 hover:text-indigo-800">+
                ...
                <div class="mt-6 pt-6 border-t">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <x-text-input id="header_cta_text" name="header[cta_button][text]" type="text"
                                class="mt-1 block w-full" :value="old('header.cta_button.text', $header['cta_button']['text'] ?? '')" />
                        </div>
                        <div>
                            <x-input-label for="header_cta_url" :value="__('Button URL')" />
                            <x-text-input id="header_cta_url" name="header[cta_button][url]" type="text"
                                class="mt-1 block w-full" :value="old('header.cta_button.url', $header['cta_button']['url'] ?? '')" />
                        </div>
                        <div class="flex items-end">
                            <label class="flex items-center">
                                <input type="checkbox" name="header[cta_button][visible]" value="1"
                                    class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500"
                                    {{ $header['cta_button']['visible'] ?? false ? 'checked' : '' }}>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6">
                ...
                <div>
                    ...
                    <textarea id="footer_about_text" name="footer[about_text]" rows="3"
                        class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('footer.about_text', $footer['about_text'] ?? '') }}</textarea>
                    ...
                    <div id="quick-links-container" class="space-y-2">
                        @foreach ($footer['quick_links'] ?? [] as $index => $link)
                            <div class="quick-link-item grid grid-cols-2 gap-2">
                                <x-text-input name="footer[quick_links][{{ $index }}][label]" type="text"
                                    class="block w-full" :value="$link['label'] ?? ''" placeholder="Label" />
                                <div class="flex gap-2">
                                    <x-text-input name="footer[quick_links][{{ $index }}][url]" type="text"
                                        class="block w-full" :value="$link['url'] ?? ''" placeholder="URL" />
                                    <button type="button"
                                        class="remove-quick-link text-red-600 hover:text-red-800 px-2">&times;</button>
                                </div>
                            </div>
                        @endforeach
                    </div>
                    <button type="button" id="add-quick-link"
                        class="mt-2 text-sm text-indigo-600 hover:text-indigo-800">+ Add quick link</button>
                </div>
                    ...
                    <div id="social-links-container" class="space-y-2">
                        @foreach ($footer['social_links'] ?? [] as $index => $link)
                            <div class="social-link-item grid grid-cols-3 gap-2">
                                <select name="footer[social_links][{{ $index }}][platform]"
                                    class="block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                    <option value="facebook"
                                        {{ ($link['platform'] ?? '') === 'facebook' ? 'selected' : '' }}>Facebook</option>
                                    <option value="twitter"
                                        {{ ($link['platform'] ?? '') === 'twitter' ? 'selected' : '' }}>Twitter</option>
                                    <option value="linkedin"
                                        {{ ($link['platform'] ?? '') === 'linkedin' ? 'selected' : '' }}>LinkedIn</option>
                                    <option value="instagram"
                                        {{ ($link['platform'] ?? '') === 'instagram' ? 'selected' : '' }}>Instagram
                                    </option>
                                    <option value="youtube"
                                        {{ ($link['platform'] ?? '') === 'youtube' ? 'selected' : '' }}>YouTube</option>
                                </select>
                                <x-text-input name="footer[social_links][{{ $index }}][url]" type="url"
                                    class="block w-full col-span-1" :value="$link['url'] ?? ''" placeholder="https://..." />
                                <button type="button"
                                    class="remove-social-link text-red-600 hover:text-red-800 px-2 justify-self-start">&times;</button>
                            </div>
                        @endforeach
                    </div>
                    <button type="button" id="add-social-link"
                        class="mt-2 text-sm text-indigo-600 hover:text-indigo-800">+ Add social link</button>
                </div>
                ...
                <div class="mt-6 pt-6 border-t">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <x-text-input id="footer_contact_address" name="footer[contact][address]" type="text"
                                class="mt-1 block w-full" :value="old('footer.contact.address', $footer['contact']['address'] ?? '')" />
                        </div>
                        <div>
                            <x-input-label for="footer_contact_phone" :value="__('Phone')" />
                            <x-text-input id="footer_contact_phone" name="footer[contact][phone]" type="text"
                                class="mt-1 block w-full" :value="old('footer.contact.phone', $footer['contact']['phone'] ?? '')" />
                        </div>
                        <div>
                            <x-input-label for="footer_contact_email" :value="__('Email')" />
                            <x-text-input id="footer_contact_email" name="footer[contact][email]" type="email"
                                class="mt-1 block w-full" :value="old('footer.contact.email', $footer['contact']['email'] ?? '')" />
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-6">
                    ...
                    <x-text-input id="footer_copyright" name="footer[copyright]" type="text"
                        class="mt-1 block w-full" :value="old('footer.copyright', $footer['copyright'] ?? '')"
