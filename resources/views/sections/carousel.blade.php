...
<section class="relative overflow-hidden" id="{{ $carouselId }}">
                ...
                <div class="relative min-h-[500px] flex items-center justify-center">
                    ...
                    <div class="relative z-10 text-center px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto">
                        ...
                        @if(!empty($slide['link_url']) && !empty($slide['link_text']))
                            <a href="{{ $slide['link_url'] }}" class="inline-flex items-center px-6 py-3 bg-white text-indigo-700 font-medium rounded-md hover:bg-gray-100 transition-colors">
                                {{ $slide['link_text'] }}
                            </a>
                        @endif
                    </div>
    ...
    @if(count($slides) > 1)
        <!-- Navigation Arrows -->
        <button class="carousel-prev absolute left-4 top-1/2 -translate-y-1/2 z-20 p-2 rounded-full bg-white/80 hover:bg-white text-gray-800 transition-colors">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
            </svg>
        </button>
        <button class="carousel-next absolute right-4 top-1/2 -translate-y-1/2 z-20 p-2 rounded-full bg-white/80 hover:bg-white text-gray-800 transition-colors">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="absolute bottom-4 left-1/2 -translate-x-1/2 z-20 flex space-x-2">
            @foreach($slides as $index => $slide)
                <button class="carousel-dot w-3 h-3 rounded-full transition-colors {{ $index === 0 ? 'bg-white' : 'bg-white/50' }}" data-index="{{ $index }}"></button>
            @endforeach
        </div>
    @endif
</section>
