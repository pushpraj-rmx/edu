                            ...
                            <div class="flex items-center">
                                ...
                                @else
                                    <div
                                        ...
                                        <span
                                            class="text-indigo-600 font-semibold text-lg">{{ substr($item['author_name'] ?? 'A', 0, 1) }}</span>
                                    </div>
                                @endif
                                <div>
                ...
                <div class="relative" id="{{ $testimonialId }}">
                            ...
                            @foreach ($items as $index => $item)
                                <div class="testimonial-item w-full flex-shrink-0 px-4">
                                    <div class="bg-white rounded-xl shadow-md p-8 max-w-3xl mx-auto text-center">
                                        <svg class="w-12 h-12 text-indigo-600 mx-auto mb-6" fill="currentColor"
                                            viewBox="0 0 24 24">
                                            <path
                                                d="M14.017 21v-7.391c0-5.704 3.731-9.57 8.983-10.609l.995 2.151c-2.432.917-3.995 3.638-3.995 5.849h4v10h-9.983zm-14.017 0v-7.391c0-5.704 3.748-9.57 9-10.609l.996 2.151c-2.433.917-3.996 3.638-3.996 5.849h3.983v10h-9.983z" />
                                        </svg>
                                        ...
                                        <div class="flex items-center justify-center">
                                            ...
                                            @else
                                                <div
                                                    class="w-14 h-14 rounded-full bg-indigo-100 flex items-center justify-center mr-4">
                                                    <span
                                                        class="text-indigo-600 font-semibold text-xl">{{ substr($item['author_name'] ?? 'A', 0, 1) }}</span>
                                                </div>
                                            @endif
                                            <div class="text-left">
                                                <p class="font-semibold text-gray-900">{{ $item['author_name'] ?? '' }}
                                                </p>
                                                @if (!empty($item['author_title']))
                                                    <p class="text-sm text-gray-500">{{ $item['author_title'] }}</p>
                                                @endif
                    ...
                    @if (count($items) > 1)
                        <div class="flex justify-center mt-8 space-x-2">
                            @foreach ($items as $index => $item)
                                <button
                                    class="testimonial-dot w-3 h-3 rounded-full transition-colors {{ $index === 0 ? 'bg-indigo-600' : 'bg-gray-300' }}"
