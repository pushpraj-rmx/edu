    ...
    <div class="max-w-4xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
        ...
        <nav class="mb-6">
            <ol class="flex items-center space-x-2 text-sm text-gray-600">
                ...
                <li>/</li>
                <li><a href="{{ route('offerings.public.index') }}" class="hover:text-gray-900">
                    {{ config('app.site_type') === 'course' ? 'Courses' : 'Services' }}
                </a></li>
                <li>/</li>
                <li class="text-gray-900">{{ $offering->title }}</li>
            </ol>
        </nav>
        ...
        <article class="bg-white rounded-lg shadow-md overflow-hidden">
            <div class="p-8">
                ...
                <!-- Title -->
                <h1 class="text-4xl font-bold text-gray-900 mb-4">{{ $offering->title }}</h1>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8 p-4 bg-gray-50 rounded-lg">
                    @if($offering->duration)
                        <div>
                            ...
                            <p class="text-gray-900">{{ $offering->duration }}</p>
                        </div>
                    @endif

                    @if($offering->eligibility)
                        <div>
                            <span class="text-sm font-semibold text-gray-700">Eligibility:</span>
                            <p class="text-gray-900">{{ $offering->eligibility }}</p>
                        </div>
                    @endif

                    @if($offering->intake)
                        <div>
                            <span class="text-sm font-semibold text-gray-700">Intake:</span>
                            <p class="text-gray-900">{{ $offering->intake }}</p>
                        </div>
                    @endif
                </div>
                ...
                @if($offering->brochure_pdf)
                    <div class="mt-8 pt-6 border-t border-gray-200">
                        <a href="{{ asset('storage/' . $offering->brochure_pdf) }}" 
                           target="_blank"
